...
def _check_auth():
    """Verify API_SECRET on write endpoints. Returns error response or None."""
    if not API_SECRET:
        return None
...
<script>
let _token=localStorage.getItem('api_token')||'';
function authHeaders(){return {'Content-Type':'application/json','Authorization':'Bearer '+_token}}
function authPost(url,body){return fetch(url,{method:'POST',headers:authHeaders(),body:body?JSON.stringify(body):undefined})}

(async function checkAuth(){
  const r=await fetch('/api/auth-check');const d=await r.json();
  if(d.auth_required && !_token){
    const t=prompt('Dashboard password:');
    if(t){_token=t;localStorage.setItem('api_token',t)}
    else{document.body.innerHTML='<h2 style="color:#ef4444;padding:40px">Access denied — password required</h2>';return}
  }
...
async function togglePause(){
  ...
  try{const r=await authPost(ep);const d=await r.json();if(d.success)_paused=d.paused;refresh()}catch(e){}
}
...
async function doWithdraw(){
  ...
  try{
    const r=await authPost('/api/withdraw',{to:addr,amount:parseFloat(amt)});
    const d=await r.json();
    if(d.error==='Unauthorized'){msg.innerHTML='<div class="msg err">Wrong password</div>';btn.disabled=false;btn.textContent='Send';return}
    if(d.success){
...
async function sellPos(tokenId,bid,sz,bp){
  ...
  try{
    const r=await authPost('/api/close',{token_id:tokenId});
    const d=await r.json();
    if(d.success){alert('Sold: '+d.message);refresh()}else{alert('Error: '+(d.error||d.message))}
...
async function cancelSell(tokenId,question){
  ...
  try{
    const r=await authPost('/api/cancel-sell',{token_id:tokenId});
    const d=await r.json();
    if(d.success){alert('Cancelled: '+d.message);refresh()}else{alert('Error: '+(d.error||d.message))}
  }catch(e){alert('Request failed')}
}
