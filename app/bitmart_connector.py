...
class BitmartConnector:
    ...
    async def _request(self, method: str, path: str, body: Optional[Dict] = None,
        ...
        url = f"{BASE_URL}{path}"
        body_str = json.dumps(body, separators=(',', ':')) if body else ""  # Compact JSON for signature
        if authenticated:
            logger.info(f"🔑 Signing: key_len={len(self.api_key)}, memo='{self.memo}', body_len={len(body_str)}, body={body_str[:100]}")
